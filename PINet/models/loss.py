...
class mixloss(nn.Module):
    def __init__(self,mu=5000,beta=0.1):
        super().__init__()
        self.l2norm = nn.MSELoss()
        self.mu = mu
        self.beta = beta
    ...
    def forward(self, input, target):
        ...
        batch_size, c, height, width = target_mu.shape
        mse_horizontal = self.l2norm(target_mu[:, :, :height - 1, :], target_mu[:, :, 1:, :])
        mse_vertical = self.l2norm(target_mu[:, :, :, :width - 1], target_mu[:, :, :, 1:])
        tv_loss = mse_horizontal + mse_vertical

        loss = loss_recon + self.beta * tv_loss
        return loss
